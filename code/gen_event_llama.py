...
def main():
    ...
    if "lemma" not in df.columns or "onset" not in df.columns:
        raise ValueError("CSV must contain 'lemma' and 'onset' columns!")
    ...
    for idx, row in df.iterrows():
        lemma_str = str(row["lemma"])
        onset_val = float(row["onset"])

        # Preprocess lemma based on language
        processed_lemma = preprocess_lemma(lemma_str, args.lang)
        ...
        inputs = tokenizer(
            ...
            padding=True,
            truncation=True,
            max_length=32  # Single word doesn't need long sequence
        )
    ...
    events_array = np.hstack([onsets_arr, embeddings_arr])  # (N, 1+hidden_dim)
